...
# --- Macro factors by year (impact on revenue and costs) ---
MACRO_FACTORS = {
    2020: {"revenue": 0.85,  "cogs_multiplier": 1.04, "dso_add": 10, "inventory_mult": 1.20, "label": "COVID_Shock"},
    2021: {"revenue": 0.93,  "cogs_multiplier": 1.02, "dso_add": 5,  "inventory_mult": 1.15, "label": "COVID_Recovery"},
    2022: {"revenue": 1.00,  "cogs_multiplier": 1.18, "dso_add": 15, "inventory_mult": 1.30, "label": "Supply_Crisis"},
    2023: {"revenue": 1.03,  "cogs_multiplier": 1.12, "dso_add": 10, "inventory_mult": 1.20, "label": "Adaptation"},
    2024: {"revenue": 1.08,  "cogs_multiplier": 1.05, "dso_add": 3,  "inventory_mult": 1.05, "label": "Normalization"},
    2025: {"revenue": 1.12,  "cogs_multiplier": 1.02, "dso_add": 0,  "inventory_mult": 1.00, "label": "Stable"},
    2026: {"revenue": 1.10,  "cogs_multiplier": 1.08, "dso_add": 5,  "inventory_mult": 1.15, "label": "Geopolitical_Risk"},
}
...
def monthly_revenue_base(year, month):
    ...
    # Macro factor
    macro = MACRO_FACTORS[year]["revenue"]
    # Seasonality
...
def generate_all():
    ...
    print("=" * 70)
    print("TechniCon Manufacturing GmbH — Data Generator v1.0")
    print("=" * 70)
    print(f"Seed: {SEED}")
    ...
    print(f"  Dimensions written: 4 files")
    print()
        ...
        for month in MONTHS:
            ...
            for div_id, div in DIVISIONS.items():
                ...
                for pg_id, pg in div_pgs.items():
                    ...
                    # COVID product impact (2020-2021)
                    if year in (2020, 2021):
                        covid_mult = COVID_PRODUCT_IMPACT[pg["covid"]]
                        # Fade COVID effect in 2021
                        if year == 2021:
                            covid_mult = 1.0 + (covid_mult - 1.0) * 0.5
            ...
            for pg_id, pg in PRODUCT_GROUPS.items():
                ...
                for comp, comp_share in cogs_component_split.items():
                    ...
                    fact_cogs_rows.append([
                        dk, scenario, div_id, pg_id, comp, mat_type,
                        qty_used, unit_cost, comp_amount
                    ])
                    month_cogs += comp_amount

                # Also generate budget line
                if year <= 2025:
                    ...
                    for comp, comp_share in cogs_component_split.items():
                        ...
                        mat_type = pg["material"] if comp == "Raw_Materials" else ""
                        fact_cogs_rows.append([
                            dk, "BUD", div_id, pg_id, comp, mat_type,
                            "", "", bud_amount
                        ])
    ...
    write_csv("fact_cogs_detail.csv",
              ["Date_Key", "Scenario_ID", "Division_ID", "Product_Group_ID",
               "Cost_Component", "Material_Type", "Quantity_Used", "Unit_Cost_EUR", "Amount_EUR"],
              fact_cogs_rows)
    ...
    for year in YEARS:
        ...
        for month in MONTHS:
            ...
            for scenario in (["ACT", "BUD"] if year <= 2025 else ["BUD"]):
                ...
                # Map to cost centers (simplified allocation)
                # Production CCs get COGS, Sales CC gets sales costs, etc.
                # For simplicity, use primary CC per category
                pnl_lines = [
                    ("PL001", gross_rev,    "CC04"),  # Gross Revenue → Sales CC
                    ("PL002", -deductions,  "CC04"),  # Sales deductions
                    ("PL010", -s_cogs * cogs_component_split["Raw_Materials"],          "CC01"),
                    ("PL011", -s_cogs * cogs_component_split["Direct_Labor"],           "CC01"),
                    ("PL012", -s_cogs * cogs_component_split["Manufacturing_Overhead"], "CC01"),
                    ("PL013", -s_cogs * cogs_component_split["Freight"],                "CC08"),
                    ("PL014", -s_cogs * cogs_component_split["Inventory_Writedown"],    "CC10"),
                    ("PL020", -sales_mktg,  "CC04"),
                    ("PL021", -gen_admin,   "CC07"),
                    ("PL022", -salaries,    "CC07"),
                    ("PL023", -rd,          "CC06"),
                    ("PL024", -da_monthly,  "CC07"),
                    ("PL025", -other_opex,  "CC09"),
                    ("PL030", -interest_exp,"CC07"),
                    ("PL031", interest_inc, "CC07"),
                    ("PL032", -abs(fx_impact), "CC07"),
                    ("PL033", -other_nonop, "CC07"),
                    ("PL040", -tax,         "CC07"),
                ]
                ...
                for div_id in DIVISIONS:
                    ...
                    for gl_id, amount, cc_id in pnl_lines:
                        allocated = round(amount * div_share, 2)
                        fact_pnl_rows.append([dk, scenario, div_id, cc_id, gl_id, allocated])
    ...
    for year in YEARS:
        ...
        for month in MONTHS:
            ...
            for gl_id, amount in bs_values.items():
                fact_bs_rows.append([dk, gl_id, amount])
            ...
            for cf_gl, cf_cat, cf_amt in cf_entries:
                fact_cf_rows.append([dk, cf_gl, cf_cat, cf_amt])
    ...
    write_csv("fact_balance_sheet.csv",
              ["Date_Key", "GL_Account_ID", "Amount_EUR"],
              fact_bs_rows)
    ...
    write_csv("fact_cashflow.csv",
              ["Date_Key", "GL_Account_ID", "CF_Category", "Amount_EUR"],
              fact_cf_rows)
    ...
    for year in YEARS:
        is_crisis = year in (2020, 2021, 2022, 2023)
        for month in MONTHS:
            dk = make_date_key(year, month)
            for seg_id, seg in CUSTOMER_SEGMENTS.items():
    ...
    expected_2024 = BASE_ANNUAL_REVENUE * ((1 + ANNUAL_ORGANIC_GROWTH) ** 5) * MACRO_FACTORS[2024]["revenue"]
    ...
    print(f"  {OUTPUT_DIR}")
    print()
    print("Next step: Open Power BI → Get Data → CSV → load all 12 files")
